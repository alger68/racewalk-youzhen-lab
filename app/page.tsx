...
import { createLocalReportStore } from "@/lib/local-report-store";
...
export default function Home(){
 ...
 const [peopleScan,setPeopleScan]=useState<PeopleScan|null>(null),[scanningPeople,setScanningPeople]=useState(false),manualAfterScan=useRef(false);
 ...
 function replaceSource(url:string){setCadence(null);queuedTarget.current=null;setAnalysisFailure("");setPeopleScan(null);if(objectUrl.current)URL.revokeObjectURL(objectUrl.current);objectUrl.current=url.startsWith("blob:")?url:"";setSrc(url);setReady(false);setTime(0);setPlaying(false);setLiveFrame(null);setPng(null);setTool(null);setFocusedEvent(null);}
 ...
 async function refresh(){setLoadingHistory(true);try{if(STATIC_MODE){const reports=createLocalReportStore(window.localStorage).list().flatMap(value=>{const parsed=reportSchema.safeParse(value);return parsed.success?[parsed.data]:[];});setHistory(reports.map(report=>({id:report.id,athlete:report.settings.athlete,session_date:report.settings.date,view:report.settings.view,speed:report.settings.speed,pace:report.settings.pace,file_name:report.file.name,summary:summarize(report),version:report.version,archived:false,updated_at:report.created})));}else{const data=await api<{sessions:HistoryRow[]}>("/api/sessions");setHistory(data.sessions);}}catch(e){setError(message(e));}finally{setLoadingHistory(false);}}
 ...
 async function save(snapshot:Report|null=r){if(!snapshot)throw new Error("尚未建立紀錄。");if(saving)throw new Error("正在儲存，請稍候。");const parsed=reportSchema.safeParse(snapshot);if(!parsed.success)throw new Error(`無法儲存：${parsed.error.issues[0]?.message}`);setSaving(true);try{const saved=STATIC_MODE?createLocalReportStore(window.localStorage).save(parsed.data):{...snapshot,version:(await api<{version:number}>("/api/sessions",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(parsed.data)})).version};setR(old=>old?.id===saved.id?{...old,version:saved.version}:old);setDirty(false);setNotice(STATIC_MODE?"分析與複查紀錄已儲存在這個瀏覽器。請另存JSON備份；原片不會上傳。":"分析與複查紀錄已儲存。原片備份可另外操作。");void refresh();return saved;}finally{setSaving(false);}}
 ...
 async function openReport(id:string){setBusy(true);try{const data=STATIC_MODE?{report:createLocalReportStore(window.localStorage).open(id),archived:false}:await api<{report:Report;archived:boolean}>(`/api/sessions/${id}`);if(!data.report)throw new Error("找不到這筆本機紀錄。");const report=reportSchema.parse(data.report);setR(report);setSetup(report.settings);setDirty(false);setArchived(data.archived);setFile(null);newFile.current=null;replaceSource(data.archived?`/api/sessions/${id}/video`:"");setTab("report");setNotice(data.archived?"紀錄與原片已載入。":STATIC_MODE?"本機紀錄已載入；回工作台選取相同原片即可疊圖。":"紀錄已載入；回工作台重新選取相同原片即可疊圖。");}finally{setBusy(false);}}
 async function remove(id:string){setBusy(true);try{if(STATIC_MODE)createLocalReportStore(window.localStorage).remove(id);else await api(`/api/sessions/${id}`,{method:"DELETE"});if(r?.id===id){setR(null);setDirty(false);setFile(null);replaceSource("");setArchived(false);}await refresh();setNotice(STATIC_MODE?"本機紀錄已刪除。":"紀錄與備份原片已刪除。");}finally{setBusy(false);}}
 ...
 return <div className="app-shell"><header className="app-header"><a className="brand" href={import.meta.env.BASE_URL} aria-label="RaceWalk Lab首頁"><span className="brand-icon"><Activity/></span><span><strong>RaceWalk <b>Lab</b></strong><small>競走動作分析</small></span></a><div className="header-right"><span className="private-label"><span className="dot"/>{STATIC_MODE?"GitHub Pages 本機分析":"私人訓練工作室"}</span><span className="avatar">RW</span></div></header>
