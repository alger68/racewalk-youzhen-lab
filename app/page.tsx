...
export default function Home(){
 ...
 return <div className="app-shell"><header className="app-header"><a className="brand" href="/" aria-label="RaceWalk Lab首頁"><span className="brand-icon"><Activity/></span><span><strong>RaceWalk <b>Lab</b></strong><small>宥蓁競走動作分析</small></span></a><div className="header-right"><span className="private-label"><span className="dot"/>私人訓練工作室</span><span className="avatar">蓁</span></div></header>
 ...
 <TabsContent value="work" forceMount className="work-tab"><div className="workspace-grid"><aside><SettingsPanel s={s} patch={patch} locked={locked} hasFrames={hasFrames}/><section className="mini-note"><Crosshair size={19}/><p>第一次先拍無痛的Easy側面。先建立個人基準，再決定要修哪一個動作。</p></section></aside><div className="workspace-main"><section className="card video-card" ref={videoCard}><div className="section-head"><div><h2>影片與動作點線</h2><p className="meta file-name">{r?.file.name??"匯入原片，開始第一筆分析"}</p></div><div className="mode-toggle"><Button size="sm" variant={mode==="bio"?"secondary":"ghost"} disabled={locked} onClick={()=>setMode("bio")}>動作分析</Button><Button size="sm" variant={mode==="judge"?"secondary":"ghost"} disabled={locked} onClick={()=>{setTool(null);setMode("judge");}}>Judge View</Button></div></div>
